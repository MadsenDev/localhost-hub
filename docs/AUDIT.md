...
- **`commands.rs` (445 LOC, all 48 command entry points): 0 tests.** This is the
  entire IPC surface — the security boundary — untested.
  *(Counts updated: 44 commands across 406 LOC at audit time; the persistence and
  credential-store work since added `list_run_history`, `read_run_log`,
  `clear_run_history` and `secret_storage_backend`.)*
...
reports 161 tests, but 66 of those are binding-export tests generated by the ts-rs
derive — they guard the type contract, not behaviour — leaving 95 hand-written Rust
